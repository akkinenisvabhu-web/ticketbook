    ...
    >
      ...
      <div className="relative bg-black/70 rounded-xl">
        <div className="relative w-full aspect-[2/3]">
          <Image src={show.image} alt={show.name} layout="fill" objectFit="cover" className="rounded-t-xl group-hover:scale-105 transition-transform duration-500 ease-in-out"/>
        ...
        <div className="p-4 bg-white/5 backdrop-blur-md border-t border-white/10">
          ...
          <Link href={`/book/${show.id}`} legacyBehavior>
            <a className={`w-full block text-center bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-bold py-2.5 px-4 rounded-lg transition-transform duration-300 ${show.ticketsLeft === 0 ? 'opacity-50 cursor-not-allowed' : 'group-hover:scale-105'}`}>
              {show.ticketsLeft === 0 ? 'Sold Out' : 'Book Tickets'}
            </a>
          </Link>
        </div>
      </div>
